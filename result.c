...
void write_iteration_statistics(char *fname, const struct ae_result *r)
{
	FILE *out;
	...
	while (left > 0) {
		...
		for (i = 0; i < n;) {
			size_t ret = fwrite(&frames[i], sizeof(frames[0]), n - i, out);
			if (ret != (n - i))
...
void ae_print_result(struct ae_mapping *map, struct ae_mapping *oldmap)
{
	...
	write_iteration_statistics(sa_output_file, r);
}
